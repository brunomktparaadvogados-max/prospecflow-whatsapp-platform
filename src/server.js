...
// O cliente só precisa fornecer: Access Token + Phone Number ID
// ══════════════════════════════════════════════════════════════════
app.post('/api/accounts', authMiddleware, async (req, res) => {
  try {
    ...
    // Valida o token conectando na Meta
    ...
    // Atualiza qualidade se disponível
    ...
    // Se tem WABA ID, sincroniza templates
    if (wabaId) {
...
// ══════════════════════════════════════════════════════════════════
// ENVIO DE MENSAGENS — Enviar para um contato
// ══════════════════════════════════════════════════════════════════

// Middleware para validar acesso à conta
  ...
  try {
    ...
    if (result.success) {
      await db.saveMessage({
        ...
        contactPhone: to.replace(/\D/g, ''), direction: 'outgoing', type: 'image',
        content: { imageUrl, caption }, status: 'sent', costCategory: 'service'
      });
    }
...
app.post('/api/accounts/:accountId/send/document', authMiddleware, accountAccess, async (req, res) => {
  try {
    ...
    if (result.success) {
      await db.saveMessage({
        accountId: req.account.id, metaMessageId: result.messageId,
        contactPhone: to.replace(/\D/g, ''), direction: 'outgoing', type: 'document',
        content: { documentUrl, filename, caption }, status: 'sent', costCategory: 'service'
      });
    }
...
app.get('/api/accounts/:accountId/templates', authMiddleware, accountAccess, async (req, res) => {
  try {
    // Se tem WABA ID, sincroniza da Meta
  ...
  try {
    ...
    if (result.success) {
      ...
      res.json({
        success: true,
        template,
        message: 'Template submetido para aprovação da Meta. Status será atualizado automaticamente.'
      });
    } else {
      res.status(400).json({
        success: false,
        error: result.friendlyError || result.error,
...
// Templates pré-montados — Atalho para criar templates comuns
    ...
    presets: [
      {
        ...
        components: [
          { type: 'BODY', text: 'Olá {{1}}! Bem-vindo(a) à {{2}}. Como podemos ajudá-lo(a) hoje?' }
        ],
      ...
      },
      {
        ...
        components: [
          { type: 'BODY', text: 'Olá {{1}}, sua reunião está confirmada para {{2}} às {{3}}. Caso precise remarcar, entre em contato.' }
        ],
      ...
      },
      {
        ...
        components: [
          { type: 'BODY', text: 'Olá {{1}}, este é um lembrete amigável sobre seu pagamento de R$ {{2}} com vencimento em {{3}}. Qualquer dúvida, estamos à disposição.' }
        ],
      ...
      },
      {
        ...
        components: [
          { type: 'BODY', text: '{{1}}, temos uma oferta especial para você! {{2}}% de desconto em {{3}}. Válido até {{4}}. Aproveite!' }
        ],
        variables: { body: ['{{name}}', '{{discount}}', '{{product}}', '{{expiry}}'] }
      },
      {
        name: 'follow_up_proposta',
        category: 'MARKETING',
        description: 'Follow-up de proposta comercial',
        components: [
          { type: 'BODY', text: 'Olá {{1}}, tudo bem? Gostaríamos de saber se teve a oportunidade de analisar nossa proposta sobre {{2}}. Ficamos à disposição para esclarecer qualquer dúvida.' }
        ],
      ...
      },
      {
        ...
        components: [
          { type: 'BODY', text: 'Seu código de verificação é: {{1}}. Válido por 10 minutos. Não compartilhe com ninguém.' }
        ],
        variables: { body: ['{{code}}'] }
      }
    ]
...
// ══════════════════════════════════════════════════════════════════
// AUTO-RESPOSTAS — Chatbot simples
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
// WEBHOOKS DO CLIENTE — Para integrar com Lovable/n8n/Flow
// ══════════════════════════════════════════════════════════════════
...
app.get('/api/docs', (req, res) => {
  ...
  res.json({
    ...
    endpoints: {
      ...
      accounts: {
        'POST /api/accounts': 'Conectar conta WhatsApp (Access Token + Phone Number ID)',
        'GET /api/accounts': 'Listar contas',
        'GET /api/accounts/:id': 'Detalhes + stats da conta',
        'PUT /api/accounts/:id/token': 'Atualizar Access Token'
      },
      messaging: {
        'POST /api/accounts/:id/send/text': 'Enviar texto (janela 24h)',
        'POST /api/accounts/:id/send/template': 'Enviar template (qualquer hora)',
      ...
      templates: {
        'GET /api/accounts/:id/templates': 'Listar templates (sincroniza com Meta)',
        'POST /api/accounts/:id/templates': 'Criar template (submete para aprovação)',
        'GET /api/templates/presets': 'Templates pré-montados prontos para usar'
      },
      contacts: {
        'GET /api/accounts/:id/contacts': 'Listar contatos (filtro por tag, busca)',
        'POST /api/accounts/:id/contacts': 'Criar/atualizar contato',
        'POST /api/accounts/:id/contacts/import': 'Importar contatos em massa'
      },
      campaigns: {
      ...
      autoReplies: {
        'GET /api/accounts/:id/auto-replies': 'Listar regras de auto-resposta',
        'POST /api/accounts/:id/auto-replies': 'Criar regra'
      },
      webhooks: {
        'GET /webhook': 'Verificação do webhook da Meta',
        'POST /webhook': 'Recebe eventos da Meta',
        'GET /api/accounts/:id/webhooks': 'Listar webhooks do cliente',
        'POST /api/accounts/:id/webhooks': 'Criar webhook (para Lovable/n8n/Flow)'
      },
      dashboard: {
        'GET /api/accounts/:id/stats': 'Estatísticas (mensagens, contatos, campanhas)'
      }
    },
    socketIO: {
      events: {
        'message_received': 'Mensagem recebida de um contato',
        'message_status': 'Status atualizado (sent, delivered, read, failed)',
        'campaign_progress': 'Progresso de campanha em tempo real'
      }
    }
  });
});
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`
╔══════════════════════════════════════════════════════════════╗
║  ProspecFlow WhatsApp Platform v1.0.0                       ║
║  Engine: Meta Cloud API (oficial)                           ║
║  Port: ${PORT}                                                  ║
║  Status: ONLINE                                             ║
║                                                             ║
